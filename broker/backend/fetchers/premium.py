...
# تابع برای وارد کردن داده‌ها به جدول premium
def insert_data_into_premium(data):
    conn = connect_to_sql_server()
    cursor = conn.cursor()

    for item in data:
        cursor.execute('''
            INSERT INTO premium (product_name, symbol, producer, contract_type, weighted_average_price, transaction_value, highest_price, supply_volume, base_price, demand_volume, transaction_volume, transaction_date, supplier, broker, lowest_price, currency, unit)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            item.get('GoodsName'), 
            item.get('Symbol'), 
            ...
            item.get('Price'),  # میانگین موزون
            item.get('TotalPrice'),  # ارزش معامله
            item.get('MaxPrice'), 
            item.get('arze'),  # عرضه
            item.get('ArzeBasePrice'),  # قیمت پایه عرضه
            item.get('taghaza'),  # تقاضا
            item.get('Quantity'),  # حجم معامله
            convert_to_gregorian(item['date']),  # تبدیل تاریخ معامله به میلادی
            item.get('ArzehKonandeh'),  # عرضه کننده
            item.get('cBrokerSpcName'),  # کارگزار
            item.get('MinPrice'),  # پایین‌ترین
...
def fetch_data_from_url(start_date_shamsi, end_date_shamsi):
    url = f'https://www.ime.co.ir/SubSystems/IME/Fiziki/AmarePermium.ashx?f={start_date_shamsi}&t={end_date_shamsi}'
    
    try:
        response = requests.get(url)

        if response.status_code == 200:
            data = response.json()  # دریافت داده‌ها از JSON
            return data
        else:
