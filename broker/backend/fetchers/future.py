...
# تابع برای وارد کردن داده‌ها به جدول future
def insert_data_into_future(data):
    conn = connect_to_sql_server()
    cursor = conn.cursor()

    for item in data:
        cursor.execute('''
            INSERT INTO future (day, contract, description, volume, value, highest_price, lowest_price, last_price, first_price, open_positions, active_brokers_count, buyers_count, sellers_count, settlement_price, settlement_change_percentage, transaction_date, delivery_date, weekly_open_positions, weekly_open_position_change, monthly_open_positions, monthly_open_position_change, last_week_settlement_price, last_week_settlement_change, last_month_settlement_price, last_month_settlement_change, previous_day_settlement_price, individual_buy_volume, individual_sell_volume, individual_buy_value, individual_sell_value)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            item.get('ContractDay'), 
            ...
            item.get('LastPrice'), 
            item.get('FirstPrice'), 
            item.get('ChangeOpenInterest'), 
            item.get('ActiveBrokers'), 
            item.get('C_Buy'), 
            item.get('C_Sell'), 
            item.get('TodaySettlementPrice'), 
            item.get('SettlementPricePercent'), 
            convert_to_gregorian(item['DT']),  # تبدیل تاریخ معامله
            convert_to_gregorian(item['DeliveryDate']),  # تبدیل تاریخ تحویل
            item.get('WeeklyOpenInterests'), 
            item.get('WeeklyOpenInterestsPercent'), 
            item.get('MonthlyOpenInterests'), 
            item.get('MonthlyOpenInterestsPercent'), 
            item.get('WeeklySettlementPrice'), 
            item.get('WeeklySettlementPricePercent'), 
            item.get('MonthlySettlementPrice'), 
            item.get('MonthlySettlementPricePercent'), 
...
# تابع برای ارسال درخواست به URL و دریافت داده‌ها
def fetch_data_from_url(start_date_shamsi, end_date_shamsi):
    url = f'https://www.ime.co.ir/subsystems/ime/futurereports/FutureAmareMoamelatHnadler.ashx?f={start_date_shamsi}&t={end_date_shamsi}&c=0&lang=8&order=asc&offset=0&limit=20000000'
    
    try:
        response = requests.get(url)

        if response.status_code == 200:
            data = response.json().get('rows', [])  # دریافت داده‌ها از قسمت "rows"
            return data
        else:
